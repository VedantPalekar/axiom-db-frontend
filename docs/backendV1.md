# AxiomDB — Technical Source of Truth
...
> **Note:** The target database container is named `axiomdb_postgres` (not `axiomdb_target_db`). All external connections use `localhost:5433`.
...
│       └── openmetadata.env
├── scripts/
│   └── seed_dirty_data.sql      # Intentionally dirty test data for axiomdb_postgres
├── data/
...
### 2.1 Internal Postgres Tables (in `axiomdb_postgres`)

All internal tables are prefixed with `_axiomdb_` and excluded from profiling scans.
...
| `table_fqn` | TEXT NOT NULL | e.g. `axiomDB.axiomdb.public.orders` |
...
| `event_id` | VARCHAR(36) NOT NULL | FK reference to `_axiomdb_events` (soft) |
...
| `profiling_report_id` | TEXT nullable | UUID of associated `_axiomdb_profiling_reports` row |
...
|---|---|---|---|
| `axiomdb:events` | `event_bus.py` | `stream_consumer.py` | Enriched webhook events |
...
```json
{
  ...
  "entityType": "testCase",
  "entityFQN": "axiomDB.axiomdb.public.orders.customer_id.orders_customer_id_not_null",
  "entity": {
    "name": "orders_customer_id_not_null",
    "fullyQualifiedName": "axiomDB.axiomdb.public.orders.customer_id.orders_customer_id_not_null",
    "testCaseResult": {
...
```json
{
  "status": "accepted",
  "event_id": "uuid",
  "table_fqn": "axiomDB.axiomdb.public.orders",
  "failed_tests": 1
}
```
...
{
  "streams": {
    "axiomdb:events": { "length": 4, "last_message_id": "..." },
    "axiomdb:repair": { "length": 1, "last_message_id": "..." },
    "axiomdb:apply":  { "length": 1, "last_message_id": "..." },
...
{
  "report_id": "uuid",
  "connection_hint": "localhost:5433/axiomdb",
  "status": "completed",
  "tables_scanned": 2,
  "total_anomalies": 6,
  "critical_count": 6,
...
{
  "connection_id": "uuid",
  "service_name": "axiomdb_localhost_axiomdb_6407e6fb",
  "connection_hint": "localhost:5433/axiomdb",
  "status": "pending",
  "tables_found": 0,
  "total_anomalies": 0,
...
```json
{
  ...
  "schema_names": ["public"],
  "status": "ready",
  "om_service_fqn": "axiomdb_localhost_axiomdb_6407e6fb",
...
Approves a proposal. Publishes `DiagnosisResult` to `axiomdb:repair` stream, triggering the repair → sandbox → apply pipeline.
...
diagnosis_agent.run(event, detector_result)
    ...
    └── confidence >= 0.70 →
            run_sandbox(event, diagnosis)  [preview run]
                │  → fetch schema + 500 rows from axiomdb_postgres
            ...
            proposal_store.create_proposal(proposal)
                │  → writes to _axiomdb_proposals (status=pending_approval)
                │
                ▼
...
Row cap: Tables with > 100,000 rows are sampled. `_axiomdb_*` tables are excluded.
...
Example: `axiomDB.axiomdb.public.orders`
...
**OM service registration note:** The OM service name must match the FQN prefix used in webhook payloads. In the current test setup, the FQN prefix is `axiomDB` (capital B) due to how the service was originally registered. The service registered programmatically via `POST /connect` uses the auto-generated name `axiomdb_{host}_{database}_{uuid8}`.
...
| `REDIS_PORT` | `6379` | Yes | |
| `REDIS_STREAM_NAME` | `axiomdb:events` | Yes | |
| `REDIS_REPAIR_STREAM` | `axiomdb:repair` | Yes | |
| `REDIS_ESCALATION_STREAM` | `axiomdb:escalation` | Yes | |
| `REDIS_APPLY_STREAM` | `axiomdb:apply` | Yes | |
| `REDIS_CONSUMER_GROUP` | `axiomdb-agents` | Yes | Shared across all agents |
...
| `CHROMA_COLLECTION` | `axiomdb_fixes` | No | |
...
| `TARGET_DB_NAME` | `axiomdb` | Yes | |
| `TARGET_DB_USER` | `axiomdb_user` | Yes | |
| `TARGET_DB_PASSWORD` | `axiomdb_pass` | Yes | |
...
The FQN prefix in webhook payloads is `axiomDB` (capital B) due to the service being registered with that capitalisation during initial setup. The `POST /connect` endpoint generates a different service name pattern (`axiomdb_{host}_{db}_{uuid}`). These two registration paths produce different FQN prefixes. Frontend must handle both.

### 7.5 Repair Agent Event Reconstruction Fallback

When the repair agent looks up the enriched event from `_axiomdb_events` by `event_id` and the record is missing (e.g. event store was unavailable at write time), it falls back to `_fallback_event_from_diagnosis()` which reconstructs column names from a hardcoded category-to-column mapping. This fallback produces incorrect column names for tables that are not `orders`.
